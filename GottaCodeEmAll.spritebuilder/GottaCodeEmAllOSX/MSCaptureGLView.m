    ...
    stdin.readabilityHandler = ^(NSFileHandle *file) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([obj objectForKey:@"start"] != nil) {
                ...
                [[CCDirector sharedDirector].responderManager mouseDown:[NSEvent mouseEventWithType:NSLeftMouseDown location:CGPointMake([[props valueForKey:@"x"] integerValue] * size.width, [[props valueForKey:@"y"] integerValue] * size.height) modifierFlags:256 timestamp:0 windowNumber:0 context:nil eventNumber:nil clickCount:0 pressure:0]];
            }
            else if ([obj objectForKey:@"up"]) {
                NSDictionary *props = (NSDictionary *)[obj objectForKey:@"up"];
                
                [[CCDirector sharedDirector].responderManager mouseDown:[NSEvent mouseEventWithType:NSLeftMouseUp location:CGPointMake([[props valueForKey:@"x"] integerValue] * size.width, [[props valueForKey:@"y"] integerValue] * size.height) modifierFlags:256 timestamp:0 windowNumber:0 context:nil eventNumber:nil clickCount:0 pressure:0]];
            }
        });
    };
}
